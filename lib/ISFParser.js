...
ISFParser.prototype.parse = function ( rawFragmentShader, rawVertexShader ) {
  try {
    ...
    this.bufferQueueSize = parseInt(metadata.BUFFER_QUEUE_SIZE)
...
ISFParser.prototype.parsePasses = function ( passesArray, persistentArray ) {
  ...
  for ( var i = 0; i < passesArray.length; ++i ) {
    ...
    pass.bufferQueueSize = parseInt(passDefinition.BUFFER_QUEUE_SIZE);
    passes.push( pass );
  }
  return passes;
}
...
ISFParser.prototype.generateShaders = function () {
  ...
  if ( this.bufferQueueSize > 0 ) {
    for (var i = 0; i < this.bufferQueueSize; i++) {
      var bufferName = "ISF_buffer_queue_" + i;
      this.addUniformLine( "uniform sampler2D " + bufferName + ";" );
      this.addUniformLine( this.samplerUniforms({NAME: bufferName}));
    }
  }
