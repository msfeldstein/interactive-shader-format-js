...
var ISFBufferQueue = require("./ISFBufferQueue.js");
...
ISFRenderer.prototype.initUniforms = function() {
  ...
  if (this.model.bufferQueueSize > 0) {
    this.bufferQueue = new ISFBufferQueue({
      frames: this.model.bufferQueueSize,
...
ISFRenderer.prototype.draw = function(destination) {
  ...
  if (this.model.bufferQueueSize > 0) {
    this.setValue("BUFFER_QUEUE_PASS", 1)
    var renderWidth = this.bufferQueue.size.width;
    var renderHeight = this.bufferQueue.size.height;
    this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, this.bufferQueue.fbo);
    this.gl.framebufferTexture2D(this.gl.FRAMEBUFFER, this.gl.COLOR_ATTACHMENT0, this.gl.TEXTURE_2D, this.bufferQueue.nextWriteFrame().texture, 0);
    ...
    this.setValue("BUFFER_QUEUE_PASS", 0)
    
    for (var i = 0; i < this.bufferQueue.buffers.length; i++) {
      var buffer = this.bufferQueue.buffers[i]
      var name = "ISF_buffer_queue_" + i
      var loc = this.program.getUniformLocation(buffer.name);
      buffer.bind(loc);
      this.setValue("_" + name + "_imgSize", [renderWidth, renderHeight]);
      this.setValue("_" + name + "_imgRect", [0, 0, 1, 1]);
      this.setValue("_" + name + "_flip", false);
...
ISFRenderer.prototype.cleanup = function() {
  ...
  if (this.bufferQueue) this.bufferQueue.destroy()
};
